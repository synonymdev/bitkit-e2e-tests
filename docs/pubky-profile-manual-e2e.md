...
> **`@pubky_profile_2`** covers edit, then persist after **terminate/relaunch** and after **wallet restore** (same seed); same **pubky** after each. Later it removes a link and tag, saves, and verifies. Avatar and limit edge cases are not in that spec. 3. Avatar:
...
- **Tags**: suite `@pubky_profile` (also `@pubky_staging` for the staging pubky shard); tests `@pubky_profile_1`, `@pubky_profile_2`, …; reserve e.g. `@pubky_ring_required` if you add Ring-specific specs later.
